...
export function createAuctionHub({ connectionString, io }) {
  ...
  async function loadPodSnapshot(podId) {
    ...
    // A lucky pod of one picks a tier instead of bidding; no clock ever runs.
    const pickMode = isPickMode({ podKind: pod.kind, podSize });
    ...
    const lots = pod.lots.map((lot) => {
      ...
      return {
        ...
        // except in pick mode, where an open lot is simply up for the taking.
        awaitingQuorum: lot.status === "OPEN" && lot.closesAt === null && !pickMode,
        nextMin: nextMinBid({
          startingBid: entryPrice,
          minIncrement: sc.minIncrement,
    ...
    return {
      ...
      shared: {
        ...
        pod: {
          ...
          quorum,
          onlineCount,
          mode: pickMode ? "PICK" : "BID",
          windowSeconds: tierWindowSeconds(pod.kind),
        },
  ...
  async function maybeStartTimers(podId) {
    const [pod, memberships, waiting, online] = await Promise.all([
      prisma.pod.findUnique({ where: { id: podId }, select: { kind: true } }),
    ...
    if (!pod || waiting.length === 0) return false;

    // A lucky pod of one has nothing to race against: its tiers stay open,
    // without a deadline, until the team picks one.
    if (isPickMode({ podKind: pod.kind, podSize: memberships })) return false;
  ...
  /**
   * Teams in the pod that still have no tier this round. Both pod kinds use
   * it: a main pod to know when it is done, a remainder pod to decide whether
   * an unbid tier may be skipped.
   */
  async function unassignedTeamCount(podId) {
    const [memberships, settled] = await Promise.all([
      prisma.podMembership.count({ where: { podId } }),
      prisma.settlement.count({ where: { lot: { podId } } }),
    ]);
    return Math.max(0, memberships - settled);
  }

  /** Open the lowest-ranked PENDING lot in a pod, if the pod has one left. */
  async function openNextLot(podId) {
    const [pod, next, unassigned] = await Promise.all([
      prisma.pod.findUnique({ where: { id: podId }, select: { kind: true } }),
      ...
      unassignedTeamCount(podId),
    ]);

    // Every team already holds a tier, so whatever is left has nobody to go
    // to. This is the normal end of a remainder pod (fewer teams than tiers);
    // a main pod runs out of tiers and teams together and never gets here.
    if (next && unassigned === 0) {
      const withdrawn = await prisma.lot.updateMany({
        where: { podId, status: { not: "CLOSED" } },
        data: { status: "CLOSED" },
      });
      for (const [lotId, entry] of deadlines) {
        if (entry.podId === podId) deadlines.delete(lotId);
      }
      devLog(podId, `Every team here has a tier — the remaining ${withdrawn.count} tier(s) are withdrawn.`);
    ...
    if (!next) {
      ...
      // Nothing else opens, so nothing else would repaint the room: without
      // this push every client keeps the snapshot from when the last tier
      // opened — its clock at 0:00 and no winner shown for the final tiers.
      await pushRoomState(podId);
      return null;
    }

    const isRemainder = pod?.kind === "REMAINDER";

    // The last tier is never bid on — it goes to whoever is still unassigned
    // in this pod, at the listed price (rulebook 6.6). A remainder pod prices
    // every tier from the main pods' averages and bids on all of them, so
    // there the cheapest tier is an ordinary claimable lot.
    if (next.subCapsule.isAutoAssigned && !isRemainder) {
    ...
    const entryPrice = isRemainder
      ? (next.frozenPrice ?? next.subCapsule.startingBid)
      : next.subCapsule.startingBid;
    const window = tierWindowSeconds(pod?.kind);
    devLog(
      podId,
      isRemainder
        ? `"${next.subCapsule.name}" is up at its fixed price of ${entryPrice} credits — ${window}s window, ${BID_TIMEOUT_SECONDS}s after each bid. A bid decides who takes it; the price does not move.`
        : `"${next.subCapsule.name}" is up at ${entryPrice} credits — ${window}s window, ${BID_TIMEOUT_SECONDS}s after each bid.`,
    );
  ...
  async function closeLot(lotId) {
    return withLotLock(lotId, async () => {
      ...
      if (!topBid) {
        // Nobody bid inside the window. A remainder pod has more tiers than
        // teams, so it may let this one go — but only while every team still
        // without a tier is guaranteed one of those left (auction-rules.mjs,
        // canSkipUnbidLot). Past that point, and always in a main pod, the
        // tier is handed to a team that still has nothing, so it isn't
        // wasted and nobody leaves empty-handed.
        let skipped = false;
        if (isRemainder) {
          const [lotsLeftAfter, unassignedTeams] = await Promise.all([
            prisma.lot.count({ where: { podId, status: "PENDING" } }),
            unassignedTeamCount(podId),
          ]);
          skipped = canSkipUnbidLot({ lotsLeftAfter, unassignedTeams });
          if (skipped) {
            ...
            devLog(
              podId,
              `No bids on "${lot.subCapsule.name}" — skipped. ${lotsLeftAfter} tier(s) left for ${unassignedTeams} team(s) still without one.`,
              "warn",
            );
          }
        }

        const assigned = skipped ? null : await assignUnbidLot(lot);
        deadlines.delete(lotId);

        if (skipped) {
          // Nothing to settle; the next tier simply comes up.
        } else if (assigned) {
          broadcast(podId, "LOT_CLOSED", {
            lotId,
            name: lot.subCapsule.name,
        ...
        // Both pod kinds run their tiers one after the other; the next one
        // opening (or POD_COMPLETE) is what repaints the room.
        await openNextLot(podId);
        await afterLotSettled(lot.pod.capsuleId, { settledPodKind: lot.pod.kind });
        return;
      }
   ...
   */
  /**
   * Tell freshly opened lucky pods they are on, and start their clocks if
   * everyone is already here. Shared by the automatic opening (main pods
   * finished) and the organiser's Start on a held pod.
   */
  async function announceRemainderOpened(capsuleId, opened) {
    await Promise.all(
      opened.podIds.map(async (podId) => {
        devLog(
          podId,
          "Main pods are finished. Your tiers are priced from their averages: each comes up in turn, a bid only decides who takes it, and a tier nobody wants is skipped while every team can still be guaranteed one.",
        );
        const started = await maybeStartTimers(podId);
  ...
  /**
   * The moment the main pods finish, every lucky pod that has not started
   * opens. openRemainderPod itself skips pods already under way, so this is
   * safe to call more than once around that moment. Returns true if a pod
   * was opened.
   */
  async function openRemainderPodsIfMainDone(capsuleId) {
    if (!(await mainPodsFinished(prisma, capsuleId))) return false;
    const opened = await openRemainderPod(prisma, capsuleId);
    if (!opened) return false;
    await announceRemainderOpened(capsuleId, opened);
    return true;
  }

  /**
   * @param {string} capsuleId
   * @param {{ settledPodKind?: "MAIN" | "REMAINDER" }} [context] which kind
   *   of pod the settlement that got us here came from. Lucky pods open only
   *   when a *main*-pod settlement completes the main pods: a lucky pod's
   *   own settlements never reopen a sibling the organiser has reset and is
   *   holding (that one opens on their Start).
   */
  async function afterLotSettled(capsuleId, { settledPodKind = "MAIN" } = {}) {
    if (advancing.has(capsuleId)) return;
    advancing.add(capsuleId);
    try {
      if (settledPodKind === "MAIN" && (await openRemainderPodsIfMainDone(capsuleId))) return;
  ...
  async function handleBid(socket, { lotId, amount }) {
    ...
    return withLotLock(lotId, async () => {
      ...
      const [membership, settled, podSize] = await Promise.all([
      ...
      const topBid = lot.bids[0] ?? null;
      const isRemainder = lot.pod.kind === "REMAINDER";
      const pickMode = isPickMode({ podKind: lot.pod.kind, podSize });
      ...
      const closesAt = computeClosesAt({
        ...
        lastBidAt: now,
        windowSeconds: tierWindowSeconds(lot.pod.kind),
      });
  ...
  /**
   * A lucky pod of one taking a tier at its frozen price. There is no bid
   * and no clock: the pick settles the lot on the spot, the pod's other
   * tiers are withdrawn, and the round is over for that team. The
   * acknowledgement has the same shape as a bid's.
   */
  async function handleClaim(socket, { lotId }) {
    const podId = socket.data.podId;
    const teamId = socket.data.teamId;

    // The pod lock rather than the lot lock: a pick touches every lot in the
    // pod, and two picks racing each other must not both settle.
    return withPodLock(podId, async () => {
      const lot = await prisma.lot.findUnique({
        where: { id: lotId },
        relationLoadStrategy: "join",
      ...
      if (!lot || lot.pod.id !== podId) {
        return { ok: false, lotId, code: "UNKNOWN", reason: "That tier is not in your pod." };
      }
      ...
      const spent = settled.reduce((total, s) => total + s.pricePaid, 0);
      const price = lot.frozenPrice ?? lot.subCapsule.startingBid;

      const verdict = validateClaim({
        lotStatus: lot.status,
        price,
        isSeatedInPod: Boolean(membership),
        hasWonInCapsule: settled.some((s) => s.capsuleId === lot.pod.capsuleId),
        remainingBalance: STARTING_BALANCE - spent,
        reserve: reserveAfter(lot.pod.capsule.key),
        pickMode: isPickMode({ podKind: lot.pod.kind, podSize }),
      });

      if (!verdict.ok) {
        devLog(podId, `Pick of "${lot.subCapsule.name}" refused for ${membership?.teamName ?? teamId}: ${verdict.reason}`, "warn");
        return { ok: false, lotId, code: verdict.code, reason: verdict.reason };
      }

      const priceSource = await remainderPriceSource(lot.pod.capsuleId, lot.subCapsuleId);
      const now = new Date();

      await prisma.$transaction([
        prisma.lot.update({
          where: { id: lotId },
          data: { status: "CLOSED", openedAt: lot.openedAt ?? now, closesAt: now },
        }),
        prisma.settlement.create({
          data: {
            lotId,
      ...
      devLog(podId, `${membership?.teamName ?? teamId} picked "${lot.subCapsule.name}" at its fixed price of ${price} credits.`);

      // The only team here now has its tier, so openNextLot withdraws the
      // rest and announces POD_COMPLETE; then the capsule may close.
      await openNextLot(podId);
      await afterLotSettled(lot.pod.capsuleId, { settledPodKind: "REMAINDER" });

      return { ok: true, lotId, pricePaid: price };
    });
  }
  ...
  async function attach(socket, { podId, teamId, email }) {
    const membership = await prisma.podMembership.findUnique({
      ...
      relationLoadStrategy: "join",
      include: { pod: { select: { capsule: { select: { status: true } } } } },
    });
    ...
    // A room exists only while its round is live. Start event draws the pods
    // for every round ahead of time, but nobody sits in one until the
    // organiser presses Start on that round — so a pod id learned earlier
    // (or a round reset back to pending) is refused here, not just hidden by
    // the UI.
    if (membership.pod.capsule.status !== "LIVE") {
      socket.emit("ROOM_ERROR", {
        message: "Waiting for the organiser to start this round.",
      });
      return false;
    }
  ...
  /**
   * Eject every socket sitting in the given pods. Used when a round stops
   * being live (reset), so a lead drops back to the bidding page's waiting
   * state rather than staying in a room whose lots no longer exist. The
   * client treats a server-side disconnect as final and re-reads its
   * context, which then has no live round to join.
   */
  async function closeRooms(podIds, message) {
    if (podIds.length === 0) return 0;
    const sockets = await io.in(podIds).fetchSockets();
    for (const s of sockets) {
      s.emit("ROOM_ERROR", { message });
      s.disconnect(true);
    }
    for (const podId of podIds) occupiedPods.delete(podId);
    return sockets.length;
  }

  /** Called after the organiser resets the whole event: no pod survives it. */
  async function onEventReset() {
    deadlines.clear();
    advancing.clear();
    closeQueue.length = 0;
    ...
    const sockets = await io.fetchSockets();
    for (const s of sockets) {
      if (typeof s.data.podId !== "string") continue;
      s.emit("ROOM_ERROR", { message: "The organiser reset the event. Waiting for it to be started again." });
      s.disconnect(true);
    }
    occupiedPods.clear();
    if (sockets.length) log(`event reset: ${sockets.length} socket(s) ejected`);
  }

  /** Called after the organiser rewinds one round to pending: its rooms close. */
  async function onCapsuleReset(capsuleId) {
    const pods = await prisma.pod.findMany({ where: { capsuleId }, select: { id: true } });
    const podIds = new Set(pods.map((pod) => pod.id));
    for (const [lotId, entry] of deadlines) {
      if (podIds.has(entry.podId)) deadlines.delete(lotId);
    }
    advancing.delete(capsuleId);

    const ejected = await closeRooms(
      [...podIds],
      "The organiser reset this round. Waiting for it to be started again.",
    );
    log(`capsule ${capsuleId} reset: ${ejected} socket(s) ejected from ${podIds.size} pod(s)`);
  }
  ...
  /**
   * The organiser took a team out of a live pod (engine
   * releaseTeamFromLivePod). The pod's clock, if any, is gone with its lots;
   * the removed lead is shown the door so it neither counts for quorum nor
   * keeps a seat it no longer has; everyone else is repainted. And because a
   * converted main pod may have been the last one holding the round up, the
   * lucky pods are opened here if the main pods are now all finished — no
   * settlement happened, so nothing else would notice.
   */
  async function onPodConverted({ capsuleId, podId, removedTeamId }) {
    for (const [lotId, entry] of deadlines) {
      if (entry.podId === podId) deadlines.delete(lotId);
    }
    ...
    let ejected = 0;
    for (const s of sockets) {
      if (s.data.teamId !== removedTeamId) continue;
      s.emit("ROOM_ERROR", { message: "The organiser has moved your team out of this pod for this round." });
      s.disconnect(true);
      ejected += 1;
    }
    if (sockets.length - ejected === 0) occupiedPods.delete(podId);
    else await pushRoomState(podId).catch(() => undefined);

    log(`pod ${podId}: team ${removedTeamId} released (${ejected} socket(s) ejected)`);

    if (advancing.has(capsuleId)) return;
    advancing.add(capsuleId);
    try {
      await openRemainderPodsIfMainDone(capsuleId);
    } catch (error) {
      log("onPodConverted failed", error?.message);
    } finally {
      advancing.delete(capsuleId);
    }
  }

  /** The organiser pressed Start on a held lucky pod (engine startRemainderPod). */
  async function onRemainderPodStarted(capsuleId, opened) {
    await announceRemainderOpened(capsuleId, opened);
  }
