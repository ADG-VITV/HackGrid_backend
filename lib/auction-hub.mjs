  ...
  async function afterLotSettled(capsuleId) {
    ...
    try {
      if (await mainPodsFinished(prisma, capsuleId)) {
        // The remainder pod opens exactly once: when the main pods are done
        // and none of its tiers has been touched yet. Its tiers then come up
        // one at a time through openNextLot, so "some still PENDING" is the
        // normal state of a running remainder pod — not a cue to open it
        // again. Opening it again would put a second tier on the clock
        // beside the running one and throw off the skip rule's count of
        // tiers still to come.
        const [pendingRemainder, startedRemainder] = await Promise.all([
          prisma.lot.count({
            where: { pod: { capsuleId, kind: "REMAINDER" }, status: "PENDING" },
          }),
          prisma.lot.count({
            where: { pod: { capsuleId, kind: "REMAINDER" }, status: { not: "PENDING" } },
          }),
        ]);

        if (pendingRemainder > 0 && startedRemainder === 0) {
