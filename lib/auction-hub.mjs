  ...
  async function closeLot(lotId) {
    return withLotLock(lotId, async () => {
      ...
      if (!topBid) {
        ...
        // Main pods repaint the room when the next tier opens; remainder-pod tiers
        // all opened together, so their room is repainted here instead.
        if (!isRemainder) await openNextLot(podId);
        else await pushRoomState(podId);
