 ...
 *
 * The login route validates the static admin roster. The organiser gate then
 * protects dashboard data and every control route from direct browser access.
 *
...
export function createAdminRouter({ prisma, hub, organiserOnly }) {
  ...
  router.post(
    ...
    route(async (req, res) => {
      ...
      if (!name || !password) {
        return res.status(400).json({ status: "error", message: "Enter your name and password." });
      }
      ...
      res.json({ status: "success", message: "Signed in.", admin: { id: admin.id, name: admin.name } });
    }),
  );
  ...
  router.post(
    ...
    route(async (req, res) => {
      ...
      const report = await addTeamToPod(prisma, req.params.key, req.params.podId, teamId);
      // In a live round the seat went into a held lucky pod whose other leads
      // may already be in the room: show them the new member.
      if (report.status === "success" && report.live) {
        await hub?.pushRoomState?.(report.podId).catch(() => undefined);
      }
      reply(res, report);
    }),
  );
  ...
  router.delete(
    ...
    route(async (req, res) => {
      ...
      const report = await removeTeamFromPod(prisma, req.params.key, req.params.podId, teamId);
      // In a live round this ejects the released lead, drops the pod's clock
      // and — if that pod was the last main pod holding the round up — opens
      // the lucky pods.
      if (report.status === "success" && report.live) {
        await hub?.onPodConverted?.(report).catch(() => undefined);
      }
      reply(res, report);
    }),
  );

  // Open a held lucky / remainder pod (one that was reset, or created while
  // the round was live) once its roster is right.
  router.post(
    "/capsules/:key/pods/:podId/start",
    organiserOnly,
    route(async (req, res) => {
      const report = await startRemainderPod(prisma, req.params.key, req.params.podId);
      if (report.status === "success") {
        await hub?.onRemainderPodStarted?.(report.capsuleId, report.opened).catch(() => undefined);
      }
      reply(res, report);
    }),
  );
